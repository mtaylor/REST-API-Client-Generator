    ...
    class Ruby
      ...
      def self.create_client(config)
        ...
        File.open("#{config['name']}.rb", "w") do |f|
          f.write "require '#{config['name']}/base.rb'\n"
        ...
        config["resources"].each do |name, options|
          create_class(name, options, config)
        end
      ...
      private
      def self.create_class(name, options, config)
        File.open("#{name}.rb", "w") do |f|
          f.write "module #{config['name'].camel_case}\n"
      ...
      def self.create_base_class(config)
        File.open("base.rb", "w") do |f|
          f.write "require 'active_resource'\n"
          f.write "module #{config['name'].camel_case}\n"
          f.write "  class Base < ::ActiveResource::Base\n"
          f.write "  end\n"
          f.write "end"
        end
      end
      ...
      def self.create_gemspec(config)
        File.open("#{config['name']}.gemspec", "w") do |f|
          ...
          f.write "  gem.files = Dir['{lib}/**/*']\n"
          f.write "  gem.require_paths = ['lib']\n"
          f.write "  gem.add_dependency 'activeresource', '~> 3.2.8'\n"
          f.write "end"
        end
      end
